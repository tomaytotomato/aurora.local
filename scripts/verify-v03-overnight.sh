...
if [ "$SKIP_BACKEND" = "1" ]; then
  ...
  info "cached expectation: 348 tests, 0 failures, 0 errors (as of iter-30 /api/audit/events)"
elif [ ! -d "$BACKEND" ]; then
  ...
  if docker run --rm \
    ...
    if [ -n "$RUN" ] && [ "$RUN" -lt 348 ]; then
      bad "test count $RUN below iter-30 baseline 348 — check for silently-removed tests"
    fi
  else
