...
# flow ever does (setting AURORA_LAUNCHED_BY=aurora-dashboard the same
# way LaunchService does for up.sh), tearing down every enabled package
# including the dashboard's own would SIGTERM the very process running
...
if [[ "${AURORA_LAUNCHED_BY:-}" == "aurora-dashboard" ]] \
  ...
  log_step "self-launch guard: excluding dashboard's own service(s) [${self_services[*]}] from 'down' (AURORA_LAUNCHED_BY=aurora-dashboard); stop it with 'down.sh dashboard' from the host instead"
fi
