#!/usr/bin/env bash
# home.local / bootstrap.sh
#
# Interactive or non-interactive installer for a home.local server.
#
# Modes:
...
# --------------------------------------------------------------------
REPO_URL="${REPO_URL:-https://github.com/tomaytotomato/home.local.git}"
...
usage() {
  cat <<EOF
home.local bootstrap v$BOOTSTRAP_VERSION
...
Environment:
  ...
  HOME_DOMAIN=home.local            Domain used for Caddy vhosts.
...
_interactive_install() {
  ...
  default_domain="${HOME_DOMAIN:-home.local}"
  ...
  domain=$(prompt_inputbox   "Domain"    "Domain for Caddy vhosts (e.g. home.local)" "$default_domain")
...
_noninteractive_install() {
  ...
  local domain="${HOME_DOMAIN:-home.local}"
