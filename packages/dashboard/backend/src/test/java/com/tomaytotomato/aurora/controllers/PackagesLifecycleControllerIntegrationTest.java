...
class PackagesLifecycleControllerIntegrationTest extends AuroraIntegrationTest {
   ...
   *
   * <p>Three lists describe "special" packages and two of them must agree:
   * {@link com.tomaytotomato.aurora.services.PackageLifecycleService#CORE_PACKAGES}
   * (403s add/start/stop/remove) and the frontend's
   * {@code CORE_PACKAGE_NAMES} in {@code src/api/packages.ts} (hides the
   * buttons). If they diverge the UI either offers an action the API
   * refuses, or hides one it would allow.
   *
   * <p>The third, {@code OnboardingService.MANDATORY_PACKAGES}, is
   * deliberately {@code {core, storage}} and must NOT be conflated with
   * these: it is what onboarding force-adds, and it leaves out
   * {@code identity} on purpose, because enabling SSO is a question the
   * wizard asks and its secrets are only generated by answering it. The
   * plan recorded this difference as "a frontend-only belief the backend
   * does not enforce", which was wrong — the enforcement is right here.
   */
  @Test
  @DisplayName("the enforced core set matches the frontend's CORE_PACKAGE_NAMES")
  void core_package_set_has_not_drifted() {
    assertThat(com.tomaytotomato.aurora.services.PackageLifecycleService.CORE_PACKAGES)
        .as("keep in step with CORE_PACKAGE_NAMES in frontend/src/api/packages.ts")
        .containsExactlyInAnyOrder("core", "identity", "storage");
  }
}
