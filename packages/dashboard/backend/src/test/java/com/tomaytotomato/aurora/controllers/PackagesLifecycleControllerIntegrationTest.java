...
class PackagesLifecycleControllerIntegrationTest extends AuroraIntegrationTest {
   ...
   *
   * <p>The third, {@code OnboardingService.MANDATORY_PACKAGES}, is now
   * {@code {core}} only (D5) and must NOT be conflated with these: it is
   * what onboarding force-adds. SSO/Authelia ships inside {@code core} and
   * is always-on, so there is no separate identity entry any more, and
   * {@code storage} is deferred to a day-2 catalogue install rather than
   * forced on at first run.
   */
  @Test
